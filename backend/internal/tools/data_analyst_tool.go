...
func claraLoadCall(filename, sheet string) string {
	if sheet != "" {
		return fmt.Sprintf("clara_load(%q, sheet=%q)", filename, sheet)
	}
	return fmt.Sprintf("clara_load(%q)", filename)
}
...
func executeDataAnalyst(args map[string]interface{}) (string, error) {
	...
	loadCall := claraLoadCall(filename, sheet)
...
# Apply Clara chart style (crisp 150-DPI, accent palette, despined)
clara_chart_style()
...
`, loadCall, filename, cleanedCode)
	...
	// Inject the robust loader so clara_load() is defined in the sandbox.
	...
	if !result.Success {
		...
		// Surface the actual schema (clara_load prints [schema] columns/dtypes at
