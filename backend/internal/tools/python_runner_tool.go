...
// clara_load('/data/report.xlsx') works (the run_python description promises
...
Files uploaded by the user are mounted at /data/<filename>. ALWAYS load them with the injected clara_load() helper instead of pd.read_csv/read_excel — it auto-handles Excel multi-sheet selection and CSV encoding/delimiter detection:
  df = clara_load('/data/sales.csv')                     # CSV/TSV — encoding + delimiter auto-detected
  df = clara_load('/data/report.xlsx')                   # Excel — auto-picks the richest sheet and prints the sheet list
  df = clara_load('/data/report.xlsx', sheet='Summary')  # Excel — a specific sheet by name
  sheets = clara_load_all('/data/report.xlsx')           # dict {sheet_name: DataFrame} for whole-workbook analysis
If a path is wrong, call _clara_list_data() to see exactly what's in /data.
...
func executePythonRunner(args map[string]interface{}) (string, error) {
	...
	if !result.Success {
		...
		// Surface stdout (incl. any [schema] columns/dtypes clara_load printed)
