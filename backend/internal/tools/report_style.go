...
func injectReportBaseCSS(html string) string {
	style := "<style id=\"clara-report-base\">" + reportBaseCSS + "</style>"
...
- For an Excel workbook, call clara_describe(path) FIRST — it prints columns +
  dtypes for EVERY sheet, so you know which sheet has which columns before you
  analyze. clara_load() also prints all sheets' columns when it auto-loads.
- Load a specific sheet with clara_load(path, sheet='Name'); clara_load_all(path)
...
────────────────────────────────────────
CHART AESTHETICS:
- Call clara_chart_style() ONCE before plotting — it sets crisp 150-DPI output,
