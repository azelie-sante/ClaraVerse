...
// defines clara_load() — a robust dataframe loader that fixes the most common
// real-world spreadsheet/CSV failure modes:
//
//   - Multi-sheet Excel: pd.read_excel(path) silently loads only the first
//     sheet. clara_load picks the richest sheet (and tells the model how to
//     target another) or clara_load_all() returns every sheet.
//   - CSV encoding: real CSVs are often Windows-1252/Latin-1/BOM, not UTF-8.
//     clara_load detects encoding (charset_normalizer) and falls back.
...
def _clara_resolve(path):
    ...
    raise FileNotFoundError("clara_load: could not find " + repr(path) + ". Files available in /data: " + repr(files))

def _clara_norm(s):
    import re
    return re.sub(r'[^a-z0-9]', '', str(s).lower())

def clara_schema(df, name=''):
...
def col(df, name):
    ...
    if name in cols:
        return name
    norm = {_clara_norm(c): c for c in cols}
    key = _clara_norm(name)
    if key in norm:
        return norm[key]
    # substring either direction
    subs = [c for c in cols if key and (key in _clara_norm(c) or _clara_norm(c) in key)]
    if len(subs) == 1:
        return subs[0]
    # word-token overlap (handles 'Monthly Salary Clean' -> 'Monthly Salary (Rs)')
    nt = _clara_toks(name)
    if nt:
        scored = sorted(((len(_clara_toks(c) & nt), c) for c in cols), reverse=True)
...
def clara_load(path=None, sheet=None, sep=None):
    """Robust loader -> pandas DataFrame; prints the exact schema after loading."""
    df = _clara_load_impl(path, sheet, sep)
    clara_schema(df, path)
    return df

def _clara_load_impl(path=None, sheet=None, sep=None):
    import os, pandas as pd
    path = _clara_resolve(path)
    ...
    if ext in ('.xlsx', '.xlsm', '.xls'):
        ...
        if sheet is None and len(names) > 1:
            ...
            print("[clara_load] Workbook has %d sheets:" % len(names))
            ...
            print("[clara_load] auto-loaded %r (richest). Use clara_load(path, sheet='Name') for another sheet, clara_describe(path) for full per-sheet schemas, or clara_load_all(path) for all." % best)
            sheet = best
...
def clara_load_all(path=None):
    """Load every sheet of an Excel workbook as {sheet_name: DataFrame}."""
    import pandas as pd
    sheets = pd.read_excel(_clara_resolve(path), sheet_name=None)
    for nm, d in sheets.items():
        clara_schema(d, 'sheet=' + str(nm))
    return sheets

def clara_describe(path=None):
    ...
    if ext in ('.xlsx', '.xlsm', '.xls'):
        ...
        for nm, d in sheets.items():
            clara_schema(d, 'sheet=' + str(nm))
        return sheets
    df = _clara_load_impl(path)
    clara_schema(df, path)
    return {'data': df}

def clara_chart_style(accent='#4F46E5'):
    """Apply Clara's modern chart aesthetics. Call once before plotting."""
    try:
