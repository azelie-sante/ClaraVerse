...
// Upstream Pi refreshed built-in provider model catalogs from pi.dev. Clara
// Agent is a fork and must not call the upstream project's infrastructure, so
// remote catalog refresh is disabled by default: built-in providers use the
// model data compiled into the package, and the ClaraVerse provider gets its
// catalog from the user's own ClaraVerse instance. Set CLARA_MODEL_CATALOG_URL
// to opt into a catalog service you control.
...
export function withRemoteCatalog(
	provider: Provider,
	catalogBaseUrl: string = process.env.CLARA_MODEL_CATALOG_URL?.trim() || DEFAULT_CATALOG_BASE_URL,
	localGeneratedAt?: number,
): Provider {
	// No catalog service configured: hand back the provider untouched so no
	// network request is ever made on its behalf.
