/**
 * RFC 8628 device-authorization polling loop.
 *
 * Vendored from Pi's own `@earendil-works/pi-ai` internal
 * `auth/oauth/device-code.ts` (MIT licensed, not part of that package's
 * public export surface by design — see the "Core only, side-effect free"
 * note in its index.ts). Copied rather than reimplemented so this extension
 * reuses the same proven polling/backoff logic the rest of Pi's built-in
 * OAuth providers (GitHub Copilot, xAI, Kimi) rely on.
 */
