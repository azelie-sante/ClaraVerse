...
// Env var names must be valid shell identifiers even when APP_NAME contains
// characters a CLI command name can have but a shell variable name can't
// (e.g. "clara-agent" -> "CLARA_AGENT").
const ENV_APP_NAME = APP_NAME.toUpperCase().replace(/[^A-Z0-9]/g, "_");

// e.g., PI_CODING_AGENT_DIR or CLARA_AGENT_CODING_AGENT_DIR
...
/**
 * Optional web viewer for shared sessions. `/share` uploads the session to the
 * user's own GitHub gist; a viewer just renders that gist client-side.
 *
 * Upstream Pi defaulted this to its own site. Clara Agent is a fork and does
 * not send users to the upstream project, and has no viewer of its own, so
 * there is no default: `/share` reports the gist URL alone unless
 * CLARA_SHARE_VIEWER_URL points at a viewer you control.
 */
export function getShareViewerUrl(gistId: string): string | undefined {
	const baseUrl = process.env.CLARA_SHARE_VIEWER_URL?.trim();
